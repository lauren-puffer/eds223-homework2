...
redlining <- st_read(here("data", "mapping-inequality","mapping-inequality-los-angeles.json"), quiet = TRUE)

# check CRS of HOLC redlining data
st_crs(redlining)

# check and transform CRS
if(crs(redlining) == crs(ej_screen_la)){
  print("Coordinate reference systems match.")
}else{
  warning("Updating coordinate reference systems to match.")
  #transform data to match
  redlining_reproj <- st_transform(redlining, st_crs(ej_screen_la))
}
```
...
birds <- st_read(here("data", "gbif-birds-LA",
                      "gbif-birds-LA.shp"), quiet = TRUE)

# check the CRS of bird data
st_crs(birds)

# check and transform CRS
if(crs(birds) == crs(redlining_reproj)){
  print("Coordinate reference systems match.")
}else{
  warning("Updating coordinate reference systems to match.")
  #transform data to match
  birds_reproj <- st_transform(birds, st_crs(redlining_reproj))
}
```
